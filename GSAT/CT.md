...
## 문제 8) 동일한 진법
2진수 X, 3진수 Y 두개의 숫자가 있다. X, Y는 각각 단 한자리만을 바꾸면 같은 수로 만들 수 있다. 예를들어 X=1010, Y=212가 주어졌을 때, 1010->1110 212->112로 바꾸면 X,Y는 14로 같은 숫자가 된다.

X와 Y가 주어질 때 같아진 숫자의 10진수를 구하시오.
...
### 풀이
일단 두 수를 십진수로 만든 다음에 2진수는 1,2,4,8,16.. , 3진수는 1,2,3,6,9,18,27...등 가까운 값을 찾아서 비교해준다.

(답 18 16 47 77 70)

## 문제 9) 고양이 찾기
고양이를 좋아하는 알잡이는 집 근처에 사는 고양이를 몇 달 동안 관찰한 결과 이 고양이가 어떤 경로로 이동하는지 모두 알게 되었다.
고양이는 무조건 한시간에 한번 이동하고, 알잡이의 예상대로 움직인다고 할 때 m시간 뒤에 고양이가 있는 위치를 구하여라.
고양이의 시작 위치는 무조건 1번이다.

고양이의 이동 방법은 다음과 같은 형식으로 주어진다.
2 4 1 5 3
이렇게 주어진다면 고양이는 1번 위치에서 2번으로, 2번 위치엣서 4번으로, 3번 위치에서 1번으로, 4번 위치에서 5번으로, 5번 위치에서 3번으로 간다는 뜻이다.

예시)
2 4 1 5 3, 8
고양이의 시작 위치는 1이고, 다음과 같이 8번 이동한다.
1>2>4>5>3>1>2>4>5
